...
from de32_3rd_team5.utils.geoutil import reverse_geo
...
with DAG(
    ...
    task_succ = BashOperator(
        task_id="a.succ",
        bash_command="""
        curl -X POST -H 'Authorization: Bearer OtM5cXkXBgCJdkhAUvaJqAszuNWESrXPATwEXMXHRZ0' -F 'message=✅ 모든 데이터를 업데이트하였습니다. ✅' https://notify-api.line.me/api/notify
        """,
    ...
    need_not_update = BashOperator(
        task_id="a.noupdate",
        bash_command="""
        curl -X POST -H 'Authorization: Bearer OtM5cXkXBgCJdkhAUvaJqAszuNWESrXPATwEXMXHRZ0' -F 'message=🚨 업데이트할 데이터가 없습니다. 🚨' https://notify-api.line.me/api/notify
		""",
    )
    error_login = BashOperator(
        task_id="e.login",
        bash_command="""
		curl -X POST -H 'Authorization: Bearer OtM5cXkXBgCJdkhAUvaJqAszuNWESrXPATwEXMXHRZ0' -F 'message=🚨 데이터베이스 서버 연결 에러 🚨\n 데이터베이스의 상태를 확인해주세요.' https://notify-api.line.me/api/notify
		""",
